...
export class FHE {
    ...
    */
	...
	public static fundamentalPeriod(
		method: 1 | 2 | 3,
		av: number,
		fv: number,
		n: number,
	...
	public static setFHEinNodes(
		frame: IFrameSystem,
		directionX: 1 | -1,
		methodPeriod: 1 | 2 | 3 = 2,
		av: number,
		fv: number,
	) {
		...
		let hmax = Math.max(
			...nodes.map((node) => node.coordinates('static').y).flat(),
		)
		let t = this.fundamentalPeriod(
			methodPeriod,
			av,
			fv,
			frame.numberLevels,
