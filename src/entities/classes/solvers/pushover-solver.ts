...
export class PushoverSolver{
    ...
    public static Run(structure: IStructure, nodeObjCoordinates: coordinates2D, stopCriteria: 'stability' | 'service', serviceLoad?: number){
        if(StaticSolver.isStable(structure)){}else{throw 'ERROR! PushoverSolver can not resolve an unestable structure'}
        let j = 0
        let nodeObj: INode
...
export const normalizeLoads2Unit = (structure: IStructure, normalizer: number) => {
    structure.elements.forEach(element => {
        element.loads.forEach(load => {
...
const stopAnalysis = (structure: IStructure, stopCriteria: 'stability' | 'service', totalLoadService?: number)=>{
