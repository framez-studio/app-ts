...
describe('Case 2: Oficial Test', () => {
	...
	cCol.assignHinge('initial',new Hinge(MnCol.maxMoment,MnCol.maxCurv,MnCol.minMoment,MnCol.minCurve,'Moment'))
	cCol.assignHinge('final',new Hinge(MnCol.maxMoment,MnCol.maxCurv,MnCol.minMoment,MnCol.minCurve,'Moment'))
	...
	let load = new RectangularSpanLoad(beam, 200)
	let load2 = new RectangularSpanLoad(beam2, 200)

	it(`Case 2: Capacity Curve`, () => {
		normalizeLoads2Unit(frm,200)
		PushoverSolver.Run(frm,{x: 0, y:3},'service',200)
