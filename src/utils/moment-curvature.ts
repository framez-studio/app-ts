...
export const my_sectionCR = (sectionCR: IRectangularSectionCR,p: number = 0) =>{
...
const eq_forces_whitney = (ci: number, sectionCR: IRectangularSectionCR, p: number = 0)=>{
...
const eq_moment_whitney = (ci: number,sectionCR: IRectangularSectionCR, p: number = 0)=>{
    let a = fun_a(ci,sectionCR.material.beta)
    let epsilon_cncr = sectionCR.material.epsilon_max
    let moment_cncr = force_whitney(a,sectionCR.material.fc,sectionCR.b) * a * 0.5
    let rowr = FMR_Reinforcement(ci,epsilon_cncr,sectionCR.reinforcement)
    let momentT = 0
...
const eq_forces_my = (ci: number, sectionCR: IRectangularSectionCR, p: number = 0)=>{
    let r = sectionCR.reinforcement.length - 1
    let epsilon_fy = sectionCR.reinforcement[r].section.fy/sectionCR.reinforcement[r].section.young
    let epsilon_cncr = absolutevalue(epsilon_fy*((ci)/(sectionCR.dmax-ci)))
    let force_cncr = 0.5 * (epsilon_cncr*sectionCR.material.young) * ci * sectionCR.b
    let rowr = FMR_Reinforcement(ci,epsilon_cncr,sectionCR.reinforcement)
    let forceT = 0
    let forceC = -force_cncr-p
    rowr.forEach(element => {
...
const eq_moment_my= (ci: number,sectionCR: IRectangularSectionCR, p: number = 0)=>{
    let r = sectionCR.reinforcement.length - 1
    let epsilon_fy = sectionCR.reinforcement[r].section.fy/sectionCR.reinforcement[r].section.young
    let epsilon_cncr = absolutevalue(epsilon_fy*((ci)/(sectionCR.dmax-ci)))
    let moment_cncr = 0.5 * (epsilon_cncr*sectionCR.material.young) * ci * sectionCR.b * (1/3) * ci
    let rowr = FMR_Reinforcement(ci,epsilon_cncr,sectionCR.reinforcement)
    let momentT = 0
    let momentC = -moment_cncr-p
    rowr.forEach(element => {
...
const FMR_Reinforcement = (ci:number, epsilon_cncr:number,reinforcement: RowReinforcement[])=>{
